...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const existingCredentials = await ExchangeCredentials.findByUserAndExchange(
        userId,
        exchangeName
      );
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message:
            'Insufficient API permissions. Please ensure your API key has read and trade permissions.'
        });
      }
...
router.get(
  '/balances',
  authenticateToken,
  requirePermission('view_balance'),
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      try {
        const credentials = await ExchangeCredentials.findByUserAndExchange(
          req.user.id,
